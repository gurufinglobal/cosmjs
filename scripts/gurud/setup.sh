...
"$gnused" -i.bak 's/prometheus = false/prometheus = true/' "$CONFIG"
"$gnused" -i.bak 's/prometheus-retention-time  = "0"/prometheus-retention-time  = "1000000000000"/g' "$APP_TOML"
"$gnused" -i.bak 's/enabled = false/enabled = true/g' "$APP_TOML"
"$gnused" -i.bak 's/enable = false/enable = true/g' "$APP_TOML"

# Change proposal periods to pass within a reasonable time for local testing
"$gnused" -i.bak 's/"max_deposit_period": "172800s"/"max_deposit_period": "30s"/g' "$GENESIS"
"$gnused" -i.bak 's/"voting_period": "172800s"/"voting_period": "30s"/g' "$GENESIS"
"$gnused" -i.bak 's/"expedited_voting_period": "86400s"/"expedited_voting_period": "15s"/g' "$GENESIS"

# set custom pruning settings
"$gnused" -i.bak 's/pruning = "default"/pruning = "custom"/g' "$APP_TOML"
"$gnused" -i.bak 's/pruning-keep-recent = "0"/pruning-keep-recent = "2"/g' "$APP_TOML"
"$gnused" -i.bak 's/pruning-interval = "0"/pruning-interval = "10"/g' "$APP_TOML"
